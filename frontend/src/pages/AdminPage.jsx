...
// Neplit - Site Control & Export Tool
const NeplitControl = () => {
  ...
  const executeCommand = async () => {
    ...
    try {
      const response = await axios.post(`${API}/neplit/execute`, { command });
      setResult(response.data.result || 'Command executed successfully');
  ...
  const downloadProject = async () => {
    setDownloading(true);
    toast.info("Preparing your standalone project...");
  ...
  const quickCommands = [
    "Change the hero headline to 'THE LEGACY AWAITS'",
    "Update the petition goal to 10,000",
    "Change the primary red color to #E31837",
    "Add a new game: NBA 2K14"
  ];

  return (
    <div data-testid="neplit-control" className="space-y-8">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-heading text-3xl font-bold text-white uppercase flex items-center gap-3">
            <Wand2 className="text-[#C8102E]" size={28} />
            Neplit
          </h2>
          <p className="text-white/60 mt-1">Edit anything with text. Export your full standalone project.</p>
        </div>
      </div>
        ...
        <div className="flex items-start justify-between gap-6">
          <div className="flex-1">
            <h3 className="font-heading text-xl font-bold text-white uppercase flex items-center gap-2">
              ...
              Download Full Standalone Project
            </h3>
            <p className="text-white/70 text-sm mt-2">
              Get your complete NBA 2K Legacy Vault as an independent package. Includes:
            </p>
            <ul className="text-white/60 text-sm mt-3 space-y-1">
              <li>• Full React frontend with all components</li>
              <li>• Complete FastAPI backend with all endpoints</li>
              <li>• MongoDB database schema & seed data</li>
              <li>• Vault AI chatbot with direct API integration</li>
              <li>• All styling, assets, and configurations</li>
      ...
      <div className="bg-[#09090B] p-6 rounded-lg border border-white/10">
        ...
        <p className="text-white/60 text-sm mb-4">
          Describe what you want to change in plain English. Examples: "Change the headline", "Update colors", "Add new content"
        </p>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {quickCommands.map((cmd, idx) => (
            <button
              ...
              className="text-xs bg-black border border-white/20 text-white/70 px-3 py-1.5 rounded hover:border-[#C8102E] hover:text-white transition-colors"
            >
              {cmd}
            </button>
          ))}
        ...
        <Textarea
          ...
          placeholder="Type your command here... e.g., 'Change the main headline to LEGACY NEVER DIES'"
          className="bg-black border-white/20 text-white min-h-[120px] mb-4"
        />
        ...
        {/* Result */}
        {result && (
          <div className="mt-4 p-4 bg-black rounded border border-white/10">
      ...
      <div className="bg-black/50 p-4 rounded-lg border border-white/10">
        <p className="text-white/50 text-xs">
          <strong className="text-white/70">Note:</strong> The downloaded project is 100% standalone. 
          Deploy it anywhere — Vercel, Railway, Render, or your own server. 
          No Emergent dependencies. Full ownership.
        </p>
      </div>
    </div>
  );
